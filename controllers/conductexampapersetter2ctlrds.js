...
exports.saveSetter = async (req, res) => {
  try {
    ...
    // Send appointment letter with user ID, password, and URL https://campustechnology.me
...
exports.sendAppointmentLetter = async (req, res) => {
  try {
    ...
    const setter = await PaperSetter.findOne({ _id: id, colid }).lean();
    if (!setter) return res.status(404).json({ success: false, message: "Paper setter not found" });

    const result = await sendAppointmentLetterEmail({
      colid,
      type: "papersetter",
      record: setter
    });
...
exports.bulkSetters = async (req, res) => {
  try {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      const data = await PaperSetter.findOneAndUpdate(
        { colid: item.colid, academicyear: item.academicyear, examcode: item.examcode, programcode: item.programcode, coursecode: item.coursecode, papersetteremail: item.papersetteremail },
        item,
        { upsert: true, new: true, setDefaultsOnInsert: true }
      );
      ...
      // Dispatch appointment letter in background for each saved setter
...
exports.getQuestionPaper = async (req, res) => {
  try {
    ...
    const [userObj, ins] = await Promise.all([
      User.findOne({ email: new RegExp(`^${setter.papersetteremail}$`, "i") }).select("name email phone designation department degree address city state pincode pan").lean(),
      getExamConfigHelper(colid)
    ]);
    res.json({ success: true, setter, paper, cos, userProfile: userObj || {}, institution: ins || {} });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.saveQuestionPaper = async (req, res) => {
  try {
    ...
    if (setter.acceptancestatus !== "Accepted") {
      return res.status(400).json({ success: false, message: "You must accept the Acceptance Form before creating or saving question papers." });
    }
...
exports.submitQuestionPaper = async (req, res) => {
  try {
    ...
    const setter = await PaperSetter.findOne({ _id: papersetterid, colid });
    if (!setter) return res.status(404).json({ success: false, message: "Paper setter assignment not found" });
    if (!setterIsActive(setter)) return res.status(400).json({ success: false, message: "Question paper submission is not active for this date range" });
    ...
    if (setter.acceptancestatus !== "Accepted") {
      return res.status(400).json({ success: false, message: "You must accept the Acceptance Form before submitting the question paper." });
    }

    const declarationdata = req.body.declarationdata;
    if (!declarationdata && setter.declarationstatus !== "Accepted") {
      return res.status(400).json({ success: false, message: "You must accept and submit the Declaration Form before submitting the question paper." });
    }
    ...
    setter.status = "InvigilatorSubmitted";
    setter.user = text(req.body.user);
    if (declarationdata) {
      setter.declarationstatus = "Accepted";
      setter.declarationdate = new Date();
...
exports.acceptForm = async (req, res) => {
  try {
    const colid = number(req.body.colid);
    const papersetterid = text(req.body.papersetterid || req.body.id);
    if (colid === undefined || !papersetterid) {
      return res.status(400).json({ success: false, message: "colid and papersetterid are required" });
    }
    const setter = await PaperSetter.findOne({ _id: papersetterid, colid });
    if (!setter) return res.status(404).json({ success: false, message: "Paper setter assignment not found" });

    const acceptancedata = req.body.acceptancedata || {};
    setter.acceptancestatus = "Accepted";
    setter.acceptancedate = new Date();
    setter.acceptancedata = acceptancedata;
    ...
    if (setter.papersetteremail) {
      await User.findOneAndUpdate(
        { email: new RegExp(`^${setter.papersetteremail}$`, "i") },
        {
          ...(acceptancedata.contact_no ? { phone: acceptancedata.contact_no } : {}),
          ...(acceptancedata.residential_address ? { address: acceptancedata.residential_address } : {})
        }
      );
    }

    res.json({ success: true, message: "Acceptance form submitted successfully", setter });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.getRemunerationBill = async (req, res) => {
  try {
    const colid = number(req.query.colid);
    const papersetterid = text(req.query.papersetterid || req.query.id);
    if (colid === undefined || !papersetterid) {
      return res.status(400).json({ success: false, message: "colid and papersetterid are required" });
    }
    const setter = await PaperSetter.findOne({ _id: papersetterid, colid }).lean();
    ...
    const rate = Number(rateCard?.papersetterrate || 0);
    const quantity = 1;
    ...
    const userObj = await User.findOne({ email: new RegExp(`^${setter.papersetteremail}$`, "i") }).lean();
    ...
    res.json({
      ...
      bill: {
        ...
        email: setter.papersetteremail,
        phone: userObj?.phone || setter.acceptancedata?.contact_no || "",
        designation: userObj?.designation || setter.acceptancedata?.designation || "Examiner",
        institute: setter.acceptancedata?.working_institute || ins?.insname || "University",
        institute_address: setter.acceptancedata?.institute_address || ins?.address || "",
        residential_address: setter.acceptancedata?.residential_address || userObj?.address || "",
        exam: setter.exam,
        examcode: setter.examcode,
        ...
        nature_of_work: "Setting of Question Paper",
        rate,
        quantity,
        totalamount,
        totalamount_words,
        bankdetails: setter.bankdetails || {
          accountholdername: setter.papersettername || "",
          panno: userObj?.pan || "",
          accountno: "",
          ifsccode: "",
          bankname: "",
        ...
        issubmitted: /^(InvigilatorSubmitted|Moderation In Progress|Moderation Submitted|Accepted)$/i.test(paper?.status || setter?.status || ""),
        status: setter.status,
        institution: {
          name: ins.institutionname || ins.insname || "",
...
exports.saveBankDetails = async (req, res) => {
  try {
    ...
    const setter = await PaperSetter.findOneAndUpdate(
      { _id: papersetterid, colid },
      { bankdetails },
      { new: true }
    );
    if (!setter) return res.status(404).json({ success: false, message: "Paper setter not found" });

    if (setter.papersetteremail && bankdetails.panno) {
      await User.findOneAndUpdate(
        { email: new RegExp(`^${setter.papersetteremail}$`, "i") },
    ...
    res.json({ success: true, message: "Bank details saved", bankdetails: setter.bankdetails });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};

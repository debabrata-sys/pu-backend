...
exports.sendAppointmentLetter = async (req, res) => {
  try {
    ...
    const mod = await Moderator.findOne({ _id: id, colid }).lean();
    if (!mod) return res.status(404).json({ success: false, message: "Moderator not found" });
...
exports.bulkModerators = async (req, res) => {
  try {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      // Dispatch appointment letter in background for each saved moderator
...
exports.getModerationPaper = async (req, res) => {
  try {
    ...
    const [userObj, ins] = await Promise.all([
      User.findOne({ email: new RegExp(`^${moderator.moderatoremail}$`, "i") }).select("name email phone designation department degree address city state pincode pan").lean(),
      getExamConfigHelper(colid)
    ]);
    res.json({ success: true, moderator, paper, audit, userProfile: userObj || {}, institution: ins || {} });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.saveModerationPaper = async (req, res) => {
  try {
    ...
    const moderator = await Moderator.findOne({ _id: moderatorid, colid }).lean();
    if (!moderator) return res.status(404).json({ success: false, message: "Moderator assignment not found" });
    if (moderator.acceptancestatus !== "Accepted") {
      return res.status(400).json({ success: false, message: "You must accept the Acceptance Form before moderating question papers." });
    }
    const paper = await findPaperForModerator(moderator);
...
exports.submitModeration = async (req, res) => {
  try {
    ...
    const moderatorid = text(req.body.moderatorid);
    if (colid === undefined || !moderatorid) return res.status(400).json({ success: false, message: "colid and moderatorid are required" });
    const moderator = await Moderator.findOne({ _id: moderatorid, colid });
    if (!moderator) return res.status(404).json({ success: false, message: "Moderator assignment not found" });
    const paper = await findPaperForModerator(moderator);
    if (!paper) return res.status(404).json({ success: false, message: "Question paper is not available" });
    if (moderator.acceptancestatus !== "Accepted") {
      return res.status(400).json({ success: false, message: "You must accept the Acceptance Form before submitting moderation." });
    }

    const declarationdata = req.body.declarationdata;
    if (!declarationdata && moderator.declarationstatus !== "Accepted") {
      return res.status(400).json({ success: false, message: "You must accept and submit the Declaration Form before submitting moderation." });
    }
    ...
    moderator.status = "Moderation Submitted";
    if (declarationdata) {
      moderator.declarationstatus = "Accepted";
      moderator.declarationdate = new Date();
    ...
    const audit = await ModerationAudit.find({ colid, moderatorid, questionpaperid: paper._id }).sort({ createdAt: -1 }).lean();
    res.json({ success: true, data, audit, moderator });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.acceptForm = async (req, res) => {
  try {
    const colid = number(req.body.colid);
    const moderatorid = text(req.body.moderatorid || req.body.id);
    if (colid === undefined || !moderatorid) {
      return res.status(400).json({ success: false, message: "colid and moderatorid are required" });
    }
    const moderator = await Moderator.findOne({ _id: moderatorid, colid });
    if (!moderator) return res.status(404).json({ success: false, message: "Moderator assignment not found" });

    const acceptancedata = req.body.acceptancedata || {};
    moderator.acceptancestatus = "Accepted";
    moderator.acceptancedate = new Date();
    moderator.acceptancedata = acceptancedata;
    if (req.body.bankdetails) {
      moderator.bankdetails = {
    ...
    if (moderator.moderatoremail) {
      await User.findOneAndUpdate(
        { email: new RegExp(`^${moderator.moderatoremail}$`, "i") },
        {
    ...
    res.json({ success: true, message: "Acceptance form submitted successfully", moderator });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.getRemunerationBill = async (req, res) => {
  try {
    const colid = number(req.query.colid);
    const moderatorid = text(req.query.moderatorid || req.query.id);
    if (colid === undefined || !moderatorid) {
      return res.status(400).json({ success: false, message: "colid and moderatorid are required" });
    }
    const moderator = await Moderator.findOne({ _id: moderatorid, colid }).lean();
    ...
    const rate = Number(rateCard?.moderatorrate || 0);
    const quantity = 1;
    ...
    const userObj = await User.findOne({ email: new RegExp(`^${moderator.moderatoremail}$`, "i") }).lean();
    ...
    res.json({
      ...
      bill: {
        ...
        phone: userObj?.phone || moderator.acceptancedata?.contact_no || "",
        designation: userObj?.designation || moderator.acceptancedata?.designation || "Moderator",
        institute: moderator.acceptancedata?.working_institute || ins?.insname || "University",
        institute_address: moderator.acceptancedata?.institute_address || ins?.address || "",
        residential_address: moderator.acceptancedata?.residential_address || userObj?.address || "",
        exam: moderator.exam,
        examcode: moderator.examcode,
        program: moderator.program,
        programcode: moderator.programcode,
        ...
        nature_of_work: "Moderation of Question Paper",
        rate,
        quantity,
        totalamount,
        totalamount_words,
        bankdetails: moderator.bankdetails || {
          accountholdername: moderator.moderatorname || "",
          panno: userObj?.pan || "",
          accountno: "",
          ifsccode: "",
          bankname: "",
        ...
        issubmitted: /^(Moderation Submitted|Accepted)$/i.test(paper?.status || moderator?.status || ""),
...
exports.saveBankDetails = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Bank details saved", bankdetails: moderator.bankdetails });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};

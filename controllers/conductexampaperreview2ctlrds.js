...
exports.getPaperDetails = async (req, res) => {
  try {
    ...
    const blocks = await BlockchainLedger.find({ colid, modelname: "conductexamquestionpaper2ds", recordid: String(paper._id) }).sort({ timestamp: -1 }).lean();
    ...
    if (paperids.length) {
      ...
      for (const selectedPaperId of paperids) {
        ...
        const block = await blockchainledgerctlrds.appendBlock({
          colid,
          modelname: "conductexamquestionpaper2ds",
          collectionname: "conductexamquestionpaper2ds",
...
exports.verifyBlockchain = async (req, res) => {
  try {
    ...
    const query = { modelname: "conductexamquestionpaper2ds" };
...
exports.formatVerifiedQuestionPaperPrint = async (req, res) => {
  try {
    ...
    const blockQuery = { modelname: "conductexamquestionpaper2ds" };
